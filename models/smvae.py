...
class Base_Decoder(nn.Module):
    def __init__(self,meta_latent_dim):
        ...
        self._cdim = cylinder_dim
        self._latent_dim = meta_latent_dim
        self._ddim = density_dim
        ...
        self._fcdi_5 = nn.Linear(n_params,2*density_dim)
        ...
        self._fcde_5 = nn.Linear(n_params,2*density_dim)
        ...
        self._fc6 = nn.Linear(n_params,4*cylinder_dim)
    ...
    def forward(self,z,h):
        ...
        di = torch.reshape(di,(-1,2,self._ddim))
        de = torch.reshape(de,(-1,2,self._ddim))
        ci,ce = c[:,:2*self._cdim],c[:,2*self._cdim:]
        ci = torch.reshape(ci,(-1,2,self._cdim))
        ce = torch.reshape(ce,(-1,2,self._cdim))
...
class Base_VAE(nn.Module):
    def __init__(self):
        ...
        self._encoder = Base_Encoder(joint_data_size,density_dim,h_dim,n_params).to(device)
        self._decoder = Base_Decoder(latent_dim,cylinder_dim ,density_dim,n_params).to(device)        
